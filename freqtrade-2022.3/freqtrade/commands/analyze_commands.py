...
def setup_analyze_configuration(args: Dict[str, Any], method: RunMode) -> Dict[str, Any]:
    ...
    """
    config = setup_utils_configuration(args, method) #get configuration data

    no_unlimited_runmodes = {
        RunMode.BACKTEST: 'backtesting', # get mode for start program execution
    }
    if method in no_unlimited_runmodes.keys():
        from freqtrade.data.btanalysis import get_latest_backtest_filename #backtesting return
 
        if 'exportfilename' in config: # exportfilename for
